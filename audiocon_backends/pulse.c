...
static void sink_info_cb(pa_context *c, const pa_sink_info *info, int eol, void *userdata)
{
    ...
    if (g_pulsedata.sink_vol_changed)
        g_pulsedata.sink_vol_changed(vol, g_pulsedata.sink_vol_changed_userdata);
}
...
{
    pa_context_get_sink_info_by_name(ctx, info->default_sink_name, sink_info_cb, userdata);
    pa_context_get_source_info_by_name(ctx, info->default_source_name, source_info_cb, userdata);
}
...
static void subscribe_callback(pa_context *ctx, pa_subscription_event_type_t type, uint32_t idx, void *userdata)
{
    ...
    case PA_SUBSCRIPTION_EVENT_SINK:
        op = pa_context_get_sink_info_by_index(ctx, idx, sink_info_cb, userdata);
        break;

    case PA_SUBSCRIPTION_EVENT_SOURCE:
        op = pa_context_get_source_info_by_index(ctx, idx, source_info_cb, userdata);
        break;
...
static void context_state_callback(pa_context *ctx, void *userdata)
{
    ...
    case PA_CONTEXT_READY:
        pa_context_get_server_info(g_pulsedata.ctx, server_info_callback, userdata);

        pa_context_set_subscribe_callback(g_pulsedata.ctx, subscribe_callback, userdata);
...
int pulsebackend_init()
{
    ...
    pa_context_set_state_callback(g_pulsedata.ctx, context_state_callback, NULL);

    if (pa_context_connect(g_pulsedata.ctx, NULL, PA_CONTEXT_NOAUTOSPAWN, NULL) < 0)
    {
...
int pulsebackend_on_output_volume_change(volume_change_cb_t cb, void *userdata)
{
    pa_threaded_mainloop_lock(g_pulsedata.mainloop);
    ...
    pa_context_get_sink_info_by_name(g_pulsedata.ctx, g_pulsedata.def_sink_name, sink_info_cb, NULL);
...
int pulsebackend_set_output_volume(u32 vol)
{
    ...
    pa_cvolume volume;
    pa_cvolume_set(&volume, g_pulsedata.def_sink_vol.channels, (float)vol * PA_VOLUME_NORM / 100);
...
int pulsebackend_on_input_volume_change(volume_change_cb_t cb, void *userdata)
{
    pa_threaded_mainloop_lock(g_pulsedata.mainloop);
    ...
    pa_context_get_source_info_by_name(g_pulsedata.ctx, g_pulsedata.def_src_name, source_info_cb, NULL);
...
int pulsebackend_set_input_volume(u32 vol)
{
    ...
    pa_cvolume volume;
    pa_cvolume_set(&volume, g_pulsedata.def_src_vol.channels, (float)vol * PA_VOLUME_NORM / 100);
