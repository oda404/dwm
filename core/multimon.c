...
{
    LTILS_CLEANUP_FREE char* mmsetup_path_norm =
        path_normalize_alloc(mmsetup_path);
    if (!mmsetup_path_norm)
        return -1;

    LTILS_CLEANUP_FCLOSE FILE* f = fopen(mmsetup_path_norm, "r");
    if (!f)
    {
        log_print(
            LOG_WARN,
            "multimon: Failed to open '%s' for reading!",
            mmsetup_path);
        return -1;
    }
