...
static atomic_bool g_screenshot_running;

static int
create_blur_window(Display *dpy, Window root, int screen)
{
    XVisualInfo vinfo;
...
static void arrange_blur_window(Display *dpy, int screen, int curx, int cury)
{
    ...
    if (curx >= g_origin_x && cury >= g_origin_y)
    {
        ...
        XFillRectangle(dpy, g_win, g_gc, g_origin_x, 0, w - g_origin_x, g_origin_y);
        XFillRectangle(dpy, g_win, g_gc, curx, g_origin_y, w - curx, h - g_origin_y);
    }
    else if (curx < g_origin_x && cury >= g_origin_y)
    {
        ...
        XFillRectangle(dpy, g_win, g_gc, g_origin_x, g_origin_y, w - g_origin_x, h - g_origin_y);
    }
    else if (curx >= g_origin_x && cury < g_origin_y)
    {
    ...
    else
    {
        ...
        XFillRectangle(dpy, g_win, g_gc, 0, g_origin_y, g_origin_x, h - g_origin_y);
        XFillRectangle(dpy, g_win, g_gc, curx, 0, w - curx, cury);
        XFillRectangle(dpy, g_win, g_gc, g_origin_x, cury, w - g_origin_x, h - cury);
    }
...
static void set_capture_origin_to_cursor(Display *dpy, Window root, int screen)
{
    ...
    XQueryPointer(dpy, root, &ret1, &ret2, &g_origin_x, &g_origin_y, &xret1, &yret1, &maskret);
...
static void save_screenshot()
{
    ...
    char cmdbuf[64];
    snprintf(cmdbuf, 64, "scrot -a %d,%d,%zu,%zu", g_origin_x, g_origin_y, w, h);
    exec_cmd(cmdbuf, NULL, 0);
}

#define EXIT_THREAD()                 \
    {                                 \
        g_screenshot_running = false; \
        return -1;                    \
    }

static int screenshot_thread(void *)
{
    g_screenshot_running = true;

    Display *dpy = XOpenDisplay(NULL);
    int screen = DefaultScreen(dpy);
    Window root = RootWindow(dpy, screen);

    /* I don't really yet get how X11 does error handling/too lazy to read man pages so no error handling for you */
    create_blur_window(dpy, root, screen);
    if (grab_keyboard(dpy, root) < 0)
        EXIT_THREAD();

    XSelectInput(dpy, g_win, PointerMotionMask | ButtonPressMask | SubstructureNotifyMask);
...
void screenshot_start(const Arg *arg)
{
    if (g_screenshot_running)
        return;

    /* We spawn another thread because running it on this thread would block the dwm loop.
    Why don't I make this a sepparate process and spawn an instance of it? idk */
    thrd_t thread;
    thrd_create(&thread, screenshot_thread, NULL);
    thrd_detach(thread);
}
