...
#define BAR_Y_PADDING (ceil(drw->fonts->h / 4.0))
#define BAR_X_PADDING (BAR_Y_PADDING)
...
int widgets_draw(Monitor* m)
{
    ...
    for (size_t i = 0; i < LENGTH(widgets); ++i)
    {
        ...
        Clr* sch = clrs_get_scheme(
            drw,
            w->fgcolor ? w->fgcolor : colors[SchemeSel][ColFg],
            w->bgcolor ? w->bgcolor : colors[SchemeSel][ColBg]);

        drw_setscheme(drw, scheme[SchemeNorm]);
        drw_rect(drw, tw, 0, tmp_tw, m->bar_height, 1, 1);
    ...
    if (widgets_width < m->widgets_width)
    {
        const size_t diff = m->widgets_width - widgets_width;
        drw_setscheme(drw, scheme[SchemeNorm]);
        drw_rect(
            drw, m->bar_width - m->widgets_width, 0, diff, m->bar_height, 1, 1);
    }

    m->widgets_width = widgets_width;
    return tw;
}

static int drawbar_tags(float dt, Monitor* m)
{
    ...
    for (size_t i = 0; i < LENGTH(tags); i++)
    {
        ...
        if (m->tagset[m->seltags] & 1 << i)
        {
            ...
            if (seltag_count == 1)
            {
                ...
                if (m->t <= 1)
                {
                    ...
                    anim_x = m->end_x * m->t + m->start_x * (1 - m->t);
                }
                else
                {
                    anim_x = m->end_x;
                ...
                drw_rect(drw, anim_x, 0, w, w, 1, 1);
            }
            else
            {
                drw_rect(drw, x, 0, w, w, 1, 1);
            }
        }

        /* The tag text */
        drw_setscheme(drw, scheme[SchemeNorm]);
        drw_text_no_bg(drw, x, 0, w, w, lrpad / 2, tags[i], 0);
        ...
        else if (occ & 1 << i) /* Has tagged clients */
        {
            drw_setscheme(drw, scheme[SchemeTagNormal]);
...
void setup(void)
{
    ...
    lrpad = drw->fonts->h;
    bh = drw->fonts->h + BAR_Y_PADDING * 4;
    updategeom();
    /* init atoms */
...
void toggleview(const Arg* arg)
{
    ...
    if (newtagset)
    {
        if (__builtin_popcount(newtagset) == 1)
        {
            int fsbidx = __builtin_ffs(newtagset) - 1;
            size_t w = 0;
            for (size_t i = 0; i < fsbidx; ++i)
                w += TEXTW(tags[i]);
...
void updatebars(void)
{
    ...
    for (m = mons; m; m = m->next)
    {
        ...
        m->bar_width = m->ww - BAR_X_PADDING * 2;
        m->bar_height = bh - BAR_Y_PADDING * 2;
...
void view(const Arg* arg)
{
    ...
    if (arg->ui & TAGMASK)
    {
        ...
        int fsbidx = __builtin_ffs(arg->ui & TAGMASK) - 1;
        size_t w = 0;
        for (size_t i = 0; i < fsbidx; ++i)
            w += TEXTW(tags[i]);

        selmon->start_x = selmon->end_x;
        selmon->end_x = w;
        selmon->t = 0;
    }
