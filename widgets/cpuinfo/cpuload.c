...
{
    return cpuinfo_to_work_time(info) + info->idle + info->iowait +
           info->guest + info->guest_nice + info->steal;
}
...
int widget_cpuload_update(struct S_Widget* w)
{
    ...
    for (size_t cpu = 0; cpu < g_cpu_count; ++cpu)
    {
        ...
        /* This way of calculation core usage has been yoinked from
         * https://github.com/GuillaumeGomez/sysinfo/blob/master/src/unix/linux/cpu.rs
         */
        ...
        float cpu_usage = rsmin(
                              cpuinfo_to_work_time(&g_cpu_new_states[cpu]),
                              cpuinfo_to_work_time(&g_cpu_old_states[cpu]),
                              0) /
                          rsmin(total_time_new, total_time_old, 1) * 100.f;
